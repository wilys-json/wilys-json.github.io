...
function addInput() {
	...
	<input class="checkbox" type="checkbox" id="alphabets-input-check-${numInputs}" checked=true>
	<br><br>`;
...
function randomize() {
	...
	let constantAlphabets = "";
	for (let i = 1; i <= numInputs; i++) {
			if (inputValues[i]) {
				const checked = document.getElementById(`alphabets-input-check-${i}`);
	...
	for (let k = 0; k < batchSize; k++) {
		...
		const constants = constantAlphabets.split(",");
		for (let i = 0; i < constants.length; i++) {
			alphabets.push(constants[i]);
		}
...
function parseCSV(csv) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		<input class="checkbox" type="checkbox" id="alphabets-input-check-${inputIndex}" checked=true>
		<br><br>`;
		inputsContainer.appendChild(newInput);
		newInput.querySelector(`#alphabets-input-${inputIndex}`).setAttribute("data-increment", inputs[1] || 1);
		var checked = newInput.querySelector(`#alphabets-input-check-${inputIndex}`);
